...
class ImportXLSXtoNodeService {
  ...
  async execute() {
    ...
    const customerFmt = (
      workSheetsFromFile[0].data[0] as Array<any>
    )[0] as string;

    const shortId = Number(
      (customerFmt.match(/\d+(\.\d)*/g) as Array<any>)[0] as number
    );
    ...
    for (let index = 8; index < workSheetsFromFile[0].data.length; index++) {
      if ((workSheetsFromFile[0].data[index] as Array<any>)[0]) {
        const itemOrd = (
          workSheetsFromFile[0].data[index] as Array<any>
        )[0] as number;
        const description = (
          workSheetsFromFile[0].data[index] as Array<any>
        )[1] as string;
        ...
        const width = Number(
          (measure.match(/(^\d+(\.\d)*)/g) as Array<any>)[0]
        );
        const height = Number(
          (measure.match(/(\d+(\.\d)*)$/g) as Array<any>)[0]
        );
